...
type EpochType struct {
	EpochName              string    // Friendly name of epoch
	EpochUses              []string  // Slice of common uses of this specific epoch
	EpochDateString        string    // The date string formatted like CustomEpochTimeFormatString that defines this
	EpochDate              time.Time // The time.Time date representation of the epoch start
	RightNowInSecondsSince int64     // time.Now() in seconds since epoch start.
	Prevalence             int       // 0-5, 0 being least common. Helps decide most likely matches when it's close.
}
...
var (
	EpochCommonEra = EpochType{
		...
		RightNowInSecondsSince: te(dateStringCommonEra),
		Prevalence: 1,
	}
	...
	EpochUnix = EpochType{
		...
		RightNowInSecondsSince: te(dateStringUnixEpoch),
		Prevalence: 5,
	}
	...
	EpochWindowsEpoch = EpochType{
		...
		RightNowInSecondsSince: te(dateStringWindowsEpoch),
		Prevalence: 5,
	}
	...
	EpochVMS = EpochType{
		...
		RightNowInSecondsSince: te(dateStringVMSEpoch),
		Prevalence: 3,
	}
	...
	EpochMicrosoftCOM = EpochType{
		...
		RightNowInSecondsSince: te(dateStringMicrosoftCOM),
		Prevalence: 4,
	}
	...
	EpochMicrosoftExcel = EpochType{
		...
		RightNowInSecondsSince: te(dateStringMicrosoftExcel),
		Prevalence: 3,
	}
	...
	EpochNTP = EpochType{
		...
		RightNowInSecondsSince: te(dateStringNTP),
		Prevalence: 2,
	}
	...
	EpochMacClassic = EpochType{
		...
		RightNowInSecondsSince: te(dateStringMacClassic),
		Prevalence: 2,
	}
	...
	EpochFAT = EpochType{
		...
		RightNowInSecondsSince: te(dateStringMicrosoftFAT),
		Prevalence: 5,
	}
	...
	EpochGPS = EpochType{
		...
		RightNowInSecondsSince: te(dateStringGPS),
		Prevalence: 2,
	}
	...
	EpochPostgreSQL = EpochType{
		...
		RightNowInSecondsSince: te(dateStringPostgreSQL),
		Prevalence: 3,
	}
	...
	EpochMacOSX = EpochType{
		...
		RightNowInSecondsSince: te(dateStringMacOSX),
		Prevalence: 5,
	}
...
func (e EpochType) String() string {
	return fmt.Sprintf("Name of Epoch: %s\n" +
		"Used for: %s\n" +
		"Started On (UTC): %s\n" +
		"Now in Epoch Seconds: %d\n", e.EpochName, strings.Join(e.EpochUses, ", "), e.EpochDate, e.RightNowInSecondsSince)
}
...
func (a ByEpochDate) Len() int           { return len(a) }
func (a ByEpochDate) Swap(i, j int)      { a[i], a[j] = a[j], a[i] }
func (a ByEpochDate) Less(i, j int) bool { return a[i].EpochDate.Second() < a[j].EpochDate.Second() }

type ByNearestDate EpochCollection

func (a ByNearestDate) Len() int           { return len(a) }
func (a ByNearestDate) Swap(i, j int)      { a[i], a[j] = a[j], a[i] }
func (a ByNearestDate) Less(i, j int) bool { return a[i].EpochDate.Second() < a[j].EpochDate.Second() }


// DateForNumber is a method on an EpochType. Given a number (in seconds), return the date (as time.Time) for the epoch
func (e *EpochType) DateForNumber(epochSeconds int64) (timeInEpoch time.Time) {
...
func (e *EpochType) NumberForDate(date time.Time) (secondsSinceEpoch int64) {
...
func secondsForEpochString(epochConstFormatString string, specificTime time.Time) (int64, error) {
	// get epoch date
	epochStart, err := time.Parse(CustomEpochTimeFormatString, epochConstFormatString)
	if err != nil {
		return 0, err
	}
	dur := int64(specificTime.Sub(epochStart).Seconds())
	return dur, err
}
...
func te(timeInRFC3339ZuluFormat string) (epoch int64) {
	epoch, _ = secondsForEpochString(timeInRFC3339ZuluFormat, time.Now())
	return epoch
}
